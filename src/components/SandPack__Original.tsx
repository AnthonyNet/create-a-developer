...
import { Exercise_1_App } from "@/exercises/Exercise_1_Data";

type SandpackCustomProps = SandpackProps & {
  customProps: { showOpenInCodeSandbox: boolean };
  options: SandpackOptions;
  mode: "preview" | "tests" | "console" | undefined;
};
...
const Sandpack = ({ customProps, options, mode }: SandpackCustomProps) => {
  ...
  const [horizontalSize, setHorizontalSize] = useState<number>(
    options.editorWidthPercentage as number
  );
...
const SandpackWrapped = ({
  ...
  return (
    <SandpackProvider
      ...
      {...props}
    >
      <Sandpack customProps={customProps} options={options} mode={mode} />
    </SandpackProvider>
  );
};

export type CustomProps = Omit<SandpackCustomProps, "mode">;

export default SandpackWrapped;
